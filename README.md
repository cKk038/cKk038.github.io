...
#### 配图是自绘的矢量网络图（按论文插图的思路画）

**不是论文里的 framework 图**（2026-09 按要求换掉了：论文插图是为具体方法服务的，
缩到卡片宽度后小字全糊，而且跟「这个方向在研究什么」对不上）。

现在的画法是**照着顶级论文插图（teaser / framework figure）的思路组织**：图里塞
**具体的缩略图内容**——任务示例、输入输出长什么样、难点在哪、结果对比——
而不是只画方框和箭头。理由很实在：卡片尺寸下文字读不出来，但形状和颜色读得出来。

| 方向 | 图里画了什么 |
|---|---|
| 多源数据智能分析 | 同一场景的五种模态缩略图（可见光 / 红外 / 深度 / SAR / 文本，红外那张标出「可见光看不见的目标」）→ 跨模态对齐与融合（注意力热图 + 共享表示）→ 三个下游任务的结果示例 |
...
海军蓝 `#0E1F38`、暖橙 `#D9822B`），和页面风格一致；体积 15–34 KB（gzip 后更小）；
动画用 CSS 关键帧，且遵循系统的「减少动态效果」设置自动静止。

四张图各有动画，**但都只改描边高亮，不改透明度** —— 淡入淡出会让内容在某些时刻
整体缺失（随手一瞥就看到空列表），和「内容要清晰」冲突，所以不用那种做法。

点云与 SAR 斑点用**固定种子的伪随机数**生成，保证每次输出的图完全一致（可复现）。

图里的文字是英文技术术语（`SAR` / `RGB` / `3D` / `6D pose` …）：站点是中英双语，
图里写字就得写两套，而这些术语在中英文技术语境里写法一致，避免为两种语言各维护一张图。
